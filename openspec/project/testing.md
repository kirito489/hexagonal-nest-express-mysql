...
| `compose-files.spec.ts` | 8 | 每份 compose 都有 script 會啟動；對外埠須寫進 README；docker 相關檔案提到的 `pnpm <script>` 須存在（掃描**遞迴**，`docker/` 開子目錄不會 `EISDIR`）；**api 與 web 不得宣告對外埠**（單一入口，以 nginx 當正對照組）；**連線類變數須在 compose 釘死**或列入具名豁免 |
...
| `permission-codes-sync.spec.ts` | 5 | 前端 `permission-codes.ts` 的碼必須存在於後端 `PERMISSION_CATALOG`；路由與 sidebar 對同一 path 的權限宣告必須一致，且 sidebar 藏了的 path 不得沒有守衛。權限樹的中文對照必須**雙向**齊全（缺對照與死條目都紅）；`SecurityController` 必須仍有 `@Roles(SUPERADMIN)`（**判定前先去註解**，否則 `// @Roles(...)` 會餵飽正規式）。**放 api 側**：前端守則用 `import.meta.glob` 讀不到 `apps/api` |
