...
- **e2e 出現過一次無法重現的失敗**：2026-08-14 在 `pnpm test` 緊接 `pnpm test:e2e` 的組合中出現 `1 failed / 137 passed`，之後單獨連跑 3 次、組合連跑 2 次皆 138 全綠，**未能重現、也未取得失敗測試名稱**（當時輸出被 grep 過濾掉了）。所有 spec 共用同一測試庫且 `--runInBand`，懷疑是連續執行下的資源競爭。下次若再出現，先用 `test:e2e 2>&1 | tee` 保留完整輸出再查。
