...
### 2026-09-06 — 靜態守則要挑「不會被運算掉」的東西當鍵

**踩到什麼**：寫「`app.use()` 掛載必須申報」的守則時，想用解析後的路徑當鍵。實際上 `main.ts` 的路徑全是運算出來的——`` `${basePath}/docs` ``、`env.LOCAL_MEDIA_BASE_URL`——**靜態掃描解析不出任何字面值**，照原計畫寫出來的守則會掃到 0 個而永遠通過。

**Why**：靜態守則只看得到原始碼文字。挑一個「執行期才知道值」的東西當判定依據，等於要求掃描器做求值——而它做不到，失敗的方式是**空清單靜默通過**。

**How to apply**：改用**第一個引數的原始碼文字**當鍵。這不是退而求其次——它更精確：改了那個表達式就會需要重新申報，而那正是該重新想一次「這條路徑可以公開嗎」的時機。通用判準：**守則的鍵要選「改動時一定會變」的東西**，而原始碼文字天生滿足這點。同一個檔案還要加「掃描數 > 0」的自我檢查，否則解析失效時看起來就像沒有違規。

### 2026-09-06 — 合成輸入的自我測試會抓出規則自己的漏洞

**踩到什麼**：`public-surface.spec.ts` 用 `app\.use\(\s*([^,)]+),` 抓第一個引數。合成輸入測到 `app.use((req, res, next) => next())` 時紅了——正規式在 `(req,` 的逗號就斷開，捕捉到 `(req`，既不是函式呼叫也不含 `=>`，於是被誤判成「未申報的路徑」。

**Why**：規則寫的當下腦中只有真實檔案裡的那幾種寫法。真實檔案剛好沒有多參數的 inline middleware，所以**只跑真實輸入的話這個缺陷永遠不會顯現**——直到有人寫了那種 middleware，而那時他會收到一個看不懂的「未申報」錯誤。

**How to apply**：字串比對型的守則一律附一組合成輸入，**而且要刻意寫幾個「應該不命中」的變體**——不命中的那半才是抓偽陽性的地方。本例補一條「以 `(` 開頭就排除」即修好。這與「守則要有掃描數 > 0」互補：那條擋偽陰性，合成輸入擋偽陽性。
...
### 2026-09-06：擴大守則範圍時，「掃到的」與「該擋的」是兩個集合

**踩到什麼**：把 `no-inline-message.spec.ts` 從 `domain/exception/` 擴到 guard / service /
infrastructure，判準沿用原本的「任何引號 + 中文字元」。跑起來 **79 處**，
而真正的違規只有二十多處。多出來的五十幾處全是**內部字串**：logger 訊息、
System Log 的 action 名、auth log 的 detail 欄位、信件主旨、Zod 欄位驗證訊息、
health indicator 的狀態描述——沒有一條會送到客戶端。

**Why**：寬判準在 `domain/exception/` 能用，是因為**那些檔案裡只有例外建構**，
沒有 log 也沒有驗證。那是那個目錄的性質，不是判準本身的正確性。範圍一擴大，
前提就沒了。當下的誘惑是「加個豁免清單」——五十幾條豁免等於規則已死，
而且每條豁免都是未來的漏洞。

**How to apply**：擴大任何靜態掃描的範圍時，**先跑一次看命中數**，再決定判準。
命中數遠超預期就是判準與意圖不對齊的訊號，不是「原來有這麼多技術債」。
不同性質的範圍可以用不同判準（本例：`domain/exception/` 維持寬判準，
其餘三層精準比對 `new \w*Exception(` 的字面值引數）。

### 2026-09-06：守則寫完要問「它擋的是不是問題的成因」

**踩到什麼**：發現 `INVALID_UPLOAD: (reason) => reason` 是恆等函式，讓五個呼叫端
各自寫文案而訊息表什麼都沒記錄。於是設計了一條「訊息表不得出現恆等函式」的守則，
連 spec 都寫好了。實作後才想清楚：**恆等函式之所以是問題，只因為呼叫端可以傳字面值**。
呼叫端一旦被掃描守住，它就只是型別上的管道，措辭已經在同一個檔案裡了。
那條規則今天就有一個偽陽性，而要滿足它得把程式碼扭成別的形狀。

**Why**：從症狀（恆等函式）直接寫規則，而不是從成因（措辭住在呼叫端）。
兩者在當下看起來一樣，因為那個症狀確實伴隨問題出現。

**How to apply**：**為了滿足規則而要扭曲程式碼時，先懷疑規則。** 問「這條擋的是成因
還是伴隨現象」——若另一條規則已經擋住成因，這條就是多的。推翻自己剛寫好的設計比
留一條會製造偽陽性的守則便宜得多；把推翻的理由寫進 design.md，下次不會再繞回來。

### 2026-09-06：回補藍本時，前端的每個 disabled 條件都要回頭確認後端規則

**踩到什麼**：把衍生專案的帳號鎖定頁搬回模板。它的表格對「已到期」的列把解鎖鈕設成
disabled，理由寫得很清楚——解鎖服務對非 `LOCKED` 回 409，按下去必定失敗。
照抄之前順手看了模板的同一支服務：**模板是 `if (status === 'NONE') throw`，
EXPIRED 是刻意放行的**（那是 C2 加的，衍生專案在那之前分支）。

而列表只列 `lockedAt != null` 的帳號，`NONE` 根本不可能出現——
也就是說照抄的話，會做出一顆**擋住每次都會成功的操作**的按鈕，
而那個操作（清掉殘留的 `lockedAt` 與失敗計數）正是這頁存在的理由之一。

**Why**：前端的停用／隱藏條件是**後端規則的鏡像**，而鏡像不會自己更新。
兩邊分屬不同 repo、不同時間點時，那條規則早就變了。
最險的是**前端測試不會發現**：它斷言「expired 時 disabled」，
而那條斷言本身就是錯的前提——測試只證明程式碼符合一個過期的假設。

**How to apply**：搬前端程式碼時，把每一個 `disabled={...}` / 條件隱藏挑出來，
問「它依據哪條後端規則？那條規則在**這個** repo 長什麼樣？」
用 grep 讀對應的 service，不要相信註解（藍本的註解是對的——對它自己的 repo 而言）。

### 2026-09-06：重構共用規則時，先用「把規則改壞」證明測試網存在

**踩到什麼（正面）**：把鎖定到期的判定從 adapter 抽成純函式給列表共用之前，
先跑一次既有測試（31 綠），再把時效改成兩倍——**2 條變紅**，證明那張網真的罩著
要動的地方。抽完之後同樣 31 綠且**斷言一字未改**。

列表接上之後再做一次：讓列表用自己那一份時效，e2e 的兩條一致性測試變紅，
症狀正是設計文件預測的「列表說鎖著、但那個人登得進去」。

**Why**：重構的定義是「行為不變」，而「測試綠」只有在**測試會因為行為改變而紅**
的前提下才代表行為沒變。不先確認這件事，綠燈可能只是因為沒有東西在看。

**How to apply**：動共用規則前，先破壞它、確認**指名的那幾條**變紅、還原。
重構後若需要修改任何斷言，那就不是重構——停下來想清楚行為到底變了什麼。

### 2026-09-06：「隱藏」與「保護」是兩件事，而兩者的差別沒有症狀

**踩到什麼**：sidebar 依 `requiredPermission` 過濾得很完整，於是看起來權限控制已經做好了。
實際上 `/members` 與 `/roles` **完全沒有路由守衛**——手動輸入網址頁面照樣渲染，
然後它打的每一支 API 被後端擋成 403，使用者看到的是空殼配一串錯誤。

**Why**：sidebar 過濾與路由守衛解決的是不同問題，但**在正常操作路徑上長得一模一樣**
——點不到的東西就等於進不去。要走到差異，得手動輸入網址，
而那正是沒有人在日常開發中會做的事。

不是安全漏洞（真正的門在後端 `PermissionsGuard`），但那句話容易變成不修的藉口：
它是體驗問題，而體驗問題不會有人報 bug，只會累積成「這系統怪怪的」。

**How to apply**：凡是「依權限決定看不看得到」的機制，都要問**它是隱藏還是保護**。
兩者都需要時就兩者都寫，並用守則綁住：本專案的
`permission-codes-sync.spec.ts` 檢查「sidebar 宣告了權限的 path 一定要有守衛」，
所以下一頁忘了掛會當場紅。

### 2026-09-06：跨 workspace 的守則要放在讀得到兩邊的那一側

**踩到什麼**：想加「前端權限碼必須存在於後端目錄」的守則，直覺是放在前端
（`apps/web/src/test/architecture.test.ts` 已經有前端守則了）。但那支刻意用
`import.meta.glob` 讀原始碼、**不用 node 的 fs**——因為 `tsconfig.app.json` 的
types 只有 `vite/client`，引入 fs 會讓前端型別環境被迫加上 node 型別。
於是它讀不到 `apps/api` 的任何東西。

**Why**：守則的所在位置取決於**它讀得到什麼**，不是它「概念上屬於誰」。
概念上這是前端的規則，但只有 api 側的 jest 同時碰得到兩個 workspace。

**How to apply**：跨 workspace 的一致性檢查放在**限制較少的那一側**。
本例放 `apps/api/test/architecture/`，用 `readFileSync` 讀 `apps/web` 的檔案。
代價是被讀的常數必須維持字面物件的寫法（跨 workspace 的 import 解不到路徑別名），
這一點要寫進被讀檔案的檔頭警告，否則下一個人重構成函式產生時會很困惑。

### 2026-09-06：提前 return 會把後面「無條件該顯示」的區塊一起吃掉

**踩到什麼**：`PermissionsField` 開頭有一段
`if (groups.length === 0) return <p>尚無可指派的權限</p>`。
新增「安全管理不可指派」區塊時把它加在 return 的 JSX 尾端——
**權限清單為空時整個區塊不會渲染**，而那正是「後台有這頁、權限設定裡找不到」
最需要被回答的情況。

是新寫的測試抓到的（`權限清單為空時仍顯示不可指派區塊`），不是看出來的。

**Why**：提前 return 表達的是「這個元件在這個情況下沒有東西可顯示」，
但需求變成「清單可能是空的，然而**另一段內容永遠都在**」時，
那個前提就不成立了。而它壞掉的方式是**少顯示東西**——
不會報錯、不會有型別問題，只有在那個特定狀態下才看得出來。

**How to apply**：往既有元件加「無條件顯示」的區塊時，
先看它有沒有提前 return，並**為每一個提前 return 的分支各寫一條測試**
確認新區塊仍在。空狀態、載入中、錯誤這三個分支最常被漏掉。
